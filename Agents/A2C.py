...
class A2C(OnPolicyAlgorithm):
    ...
    def __init__(
        ...
        super().__init__(
            ...
            only_intrinsic_reward=only_intrinsic_reward,
    ...
    def train(self) -> None:
        ...
        for rollout_data in self.rollout_buffer.get(batch_size=None):
            ...
            # ICM Loss calculation (Modified)
            if self.icm_model is not None:
                observations, actions, next_observations = self.icm_buffer.get_triplets()
                predicted_actions, predicted_states, next_states =\
                self.icm_model.forward(observations, actions, next_observations)
        ...
        self.wandb_logger.LogLosses(calculate_grad_norm(self.policy),                       #
                               policy_loss.item(), value_loss.item(),                       #
                               entropy_loss.item(), loss.item(),                            # Modified
                               icm_loss=icm_loss_value,                                     #
                               icm_grad_norm=calculate_grad_norm(self.icm_model))           #
