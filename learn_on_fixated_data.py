...
path_to_file = "/home/dvasilev/mario_icm/mario_buffer_{}.pkl"
for i in range(2):
    print(i)
    torch.cuda.empty_cache()
    with open(path_to_file.format(i), "rb") as pickled_buffer:
        subbuffer = pickle.load(pickled_buffer)
        for i in range(len(subbuffer)):
            item0 = subbuffer[i][0].cpu()
            item1 = subbuffer[i][1].cpu()
            item2 = subbuffer[i][2].cpu()
            subbuffer[i] = (item0, item1, item2)
        icm_buffer.buffer += subbuffer

def train(model, optimizer, buffer):
    observations, actions, next_observations = buffer.get_triplets()
    observations = observations.to(ENV_CFG.DEVICE)
    actions = actions.to(ENV_CFG.DEVICE)
    next_observations = next_observations.to(ENV_CFG.DEVICE)
    ...
    #print(action_one_hot.argmax(dim=1))
        
    action_prediction_loss =\
        CE_loss(predicted_actions, action_one_hot.argmax(dim = 1))
    print(action_prediction_loss)
